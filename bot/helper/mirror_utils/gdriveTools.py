...
class GoogleDriveHelper:
    ...
    @retry(wait=wait_exponential(multiplier=2, min=3, max=6), stop=stop_after_attempt(3),
           retry=retry_if_exception_type(HttpError), before=before_log(LOGGER, logging.DEBUG))
    def copyFileForMultiple(self, file_id, dest_id):
        body = {
            'parents': [dest_id]
        }
        try:
            ...
            if err.resp.get('content-type', '').startswith('application/json'):
                ...
                if reason == 'userRateLimitExceeded' or reason == 'dailyLimitExceeded':
                    if USE_SERVICE_ACCOUNTS:
                        self.switchServiceAccount()
                        return self.copyFileForMultiple(file_id, dest_id)
                    else:
                        LOGGER.info(f"Warning: {reason}")
                        raise err
                else:
    ...
    def cloneForMultiple(self, link):
        self.transferred_size = 0
        self.total_files = 0
        self.total_folders = 0
        try:
        ...
        try:
            meta = self.getFileMetadata(file_id)
            file = self.copyFileForMultiple(meta.get('id'), parent_id)
            ...
            try:
                msg += f' [{get_readable_file_size(int(meta.get("size", 0)))}]'
                #msg += f'<br><b>Type: </b>{typ}'
            except TypeError:
                pass
            if DRIVE_INDEX_URL is not None:
                url_path = requests.utils.quote(f'{file.get("name")}', safe='')
                url = f'{DRIVE_INDEX_URL}/{url_path}'
                #url = requests.utils.requote_uri(f'{DRIVE_INDEX_URL}/{file.get("name")}')
        except Exception as err:
            if isinstance(err, RetryError):
                LOGGER.info(f"Total attempts: {err.last_attempt.attempt_number}")
            ...
            if "User rate limit exceeded" in str(err):
                ...
                if token_service is not None:
                    self.__service = token_service
                    return self.cloneForMultiple(link),'',''
                msg = "No such file exists"
            else:
                msg = str(err)
            LOGGER.error(f"{msg}")
    ...
    def drive_query(self, DRIVE_ID, search_type, file_name):
        ...
        if " " in file_name:
            ffname = file_name.split(" ")
            query = f"name contains '{ffname[0]}'"
            for i in range(1, len(ffname)):
                query += f" and name contains '{ffname[i]}'"
            query += " and "
    ...
    def drive_list(self, file_name):
        ...
        for i in range(total_pages):
            ...
            try:
                self.path.append(
                    telegraph[acc_no].create_page(title='Search Results',
